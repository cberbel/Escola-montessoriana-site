...
export const NavbarFr: React.FC = () => {
  ...
  const navLinks = [
    { name: 'La Méthode', to: '/fr/methode-montessori' },
    { name: 'Bienveillance', to: '/fr/bienveillance' },
    { name: 'Le Cerveau', to: '/fr/cerveau-de-votre-enfant' },
    { name: "L'Anglais", to: '/fr/immersion-anglais' },
    { name: 'La Nature', to: '/fr/nature-education-cosmique' },
    { name: 'Les Ambiances', to: '/fr/ambiances' },
    { name: 'Contact', to: '/fr#contact' },
    { name: 'PT', to: '/' },
    { name: 'EN', to: '/en' },
    { name: 'ES', to: '/es' },
  ];
    ...
    >
      <nav className="flex flex-col py-2 pb-[max(1.5rem,env(safe-area-inset-bottom,1.5rem))]">
        {navLinks.map((link) => (
          ...
          >
            {link.name === 'PT' ? 'Português (PT)' : link.name === 'EN' ? 'English (EN)' : link.name === 'ES' ? 'Español (ES)' : link.name}
          </Link>
        ))}
        <a
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex justify-between items-center min-w-0 gap-4">
        <Link to="/fr" className="flex items-center space-x-2 flex-shrink-0">
           {!logoError ? (
             <img
