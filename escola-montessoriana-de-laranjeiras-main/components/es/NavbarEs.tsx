...
const WHATSAPP_VISIT_ES =
  'https://wa.me/5521992973454?text=Hola%2C%20me%20gustar%C3%ADa%20agendar%20una%20visita%20a%20la%20Escola%20Montessoriana%20de%20Laranjeiras.';

export const NavbarEs: React.FC = () => {
  ...
  const navLinks = [
    { name: 'El Método', to: '/es/metodo-montessori' },
    { name: 'Acogida', to: '/es/acogida' },
    { name: 'El cerebro infantil', to: '/es/el-cerebro-de-tu-hijo' },
    { name: 'Inglés', to: '/es/inmersion-en-ingles' },
    { name: 'Naturaleza', to: '/es/naturaleza-educacion-cosmica' },
    { name: 'Grupos', to: '/es/grupos' },
    { name: 'Contacto', to: '/es#contact' },
    { name: 'PT', to: '/' },
    { name: 'EN', to: '/en' },
    { name: 'FR', to: '/fr' },
  ];
    ...
    >
      <nav className="flex flex-col py-2 pb-[max(1.5rem,env(safe-area-inset-bottom,1.5rem))]">
        {navLinks.map((link) => (
          ...
          >
            {link.name === 'PT' ? 'Português (PT)' : link.name === 'EN' ? 'English (EN)' : link.name === 'FR' ? 'Français (FR)' : link.name}
          </Link>
        ))}
        <a
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex justify-between items-center min-w-0 gap-4">
        <Link to="/es" className="flex items-center space-x-2 flex-shrink-0">
           {!logoError ? (
             <img
